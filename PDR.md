...
### プロジェクト機能への移行ステージ（2026-08-16 決定）

既存の「実験がユーザー直下にフラットにぶら下がる」構造からの移行は、以下の4段階に分けて進める。Stage 2 は破壊的変更（`experiments.project_id`が`NOT NULL`になり、実験の作成パスが変わる）なので、API とフロントを同時に直す必要がある。
...
| 3 | フロントエンドのプロジェクト画面（トップページ`/`をプロジェクトのダッシュボードにする・`/projects/{id}`配下の実験一覧と作成フォーム、ヘッダー導線） | 完了（2026-09-20） |
| 4 | 実験のコピー機能（`POST /api/v1/experiments/{id}/copy`＋コピー先を選ぶ UI） | 完了（2026-09-20。API と `/experiments/{id}` のコピー先選択 UI の両方） |
...
**プロジェクト一覧は独立したルート（`/projects`）を作らず、トップページ`/`がそれを兼ねる。** ダッシュボードから対象のプロジェクトを開き、そこで実験を追加していく、という流れにしたため。トップページ下部には「保存せずグラフだけ描く」下書き欄を置く（DB には何も書かない、未ログインでも使える）。
