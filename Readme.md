...
- 他のプロジェクトへ実験を移す手段はまだありません（移動APIが無いため）。コピー（`POST /api/v1/experiments/{id}/copy`）はAPIとしては実装済みですが、UIは未実装です
...
- `/experiments`で保存済みの実験をプロジェクト横断で一覧できます（作成日時の新しい順、タイトルと作成日を表示、クリックで`/experiments/{id}`へ）。ヘッダーの「すべての実験」とダッシュボードの「すべての実験を見る」から遷移します。1件も無い場合は空状態メッセージを表示
