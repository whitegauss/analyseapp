...
### 同じものを2回実装したとき: 共有フィクスチャで縛る

式言語（`a*exp(-x/tau)` のような手入力の数式）は**2箇所に実装がある**。フロントの誤差伝播ツール（`frontend/lib/formula.ts`、自動微分付き）と、Worker の曲線フィット（`backend/worker/app/formula.py`、numpy 配列で評価）。

**同じ式が両方で同じ意味にならなければ、アプリの2箇所で同じ文字列が別のものを指す。** そこで `shared/formula-grammar.json` を唯一の正とし、両方のテストがこれを読む（`lib/formulaGrammar.test.ts` と `tests/test_formula.py`）。片方だけ直せば、もう片方のテストが落ちる。

固定してあるのは、評価結果（NaN・Infinity を含む）、変数名の並び、**構文エラーのキャレット位置**、関数と定数の一覧。

なぜ2つ要るのか: **Python の `ast` モジュールは使えない。** `^` をべき乗として扱う文法なのに、`ast.parse` はこれをビット XOR と読む。`x^2` が例外も出さずに別の意味になるので、手書きパーサにするしかなかった（副産物として、ユーザー入力の near に `eval` が一切無い）。

同じ判断が要る場面は他にもあり得る。**実装を2つ持つと決めたなら、その場で両者を縛るフィクスチャも一緒に作ること。** 後から追加しようとすると、その時点で既にずれている。
